	...
	for pipeline in workloadDesc:
		...
		for bof in pipeline:
			...
			for i in range(0,bof):
				cud = CUDef.createTAUGromacsCU(nthreads) ## The number of cores per CU has been set to 1. (hard coded, can be changed). The creation of the CU could stay outside the loop since all the CUs are the same
